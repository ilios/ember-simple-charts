...
class SimpleChart extends Component {
  ...
  handleHover = async (data, tooltipTarget) => {
    await timeout(DEBOUNCE_MS, this);
    if (this.args.hover) {
      assert('@hover must be a function', typeof this.args.hover === 'function');
      try {
  ...
  handleLeave = async () => {
    await timeout(DEBOUNCE_MS, this);
    if (this.args.leave) {
      assert('@leave must be a function', typeof this.args.leave === 'function');
      try {
  ...
  handleClick = async data => {
    ...
    if (this.args.onClick) {
      assert('@onClick must be a function', typeof this.args.onClick === 'function');
      try {
  ...
  static {
    setComponentTemplate(precompileTemplate("<div class=\"simple-chart\" {{onResize this.calculateSize}}>\n  <this.chartComponent @data={{@data}} @isIcon={{this.isIcon}} @hover={{this.handleHover}} @onClick={{this.handleClick}} @leave={{this.handleLeave}} @isClickable={{this.isClickable}} @containerHeight={{this.height}} @containerWidth={{this.width}} @textIsNotOutlined={{this.textIsNotOutlined}} ...attributes />\n  {{#if this.tooltipTarget}}\n    {{yield (hash tooltip=(component SimpleChartTooltip target=this.tooltipTarget title=null))}}\n  {{/if}}\n</div>", {
      strictMode: true,
      scope: () => ({
        onResize,
        hash,
