...
class SimpleChartHorzBar extends Component {
  ...
  draw(element, [elementHeight, elementWidth, data]) {
    ...
    const xScale = scaleLinear().domain([0, Math.max(...dataOrArray.map(d => d.data))]).range([0, this.args.isIcon ? 100 : 95]);
    const yScale = scaleBand().domain(dataOrArray.map(d => d.label)).range([0, this.args.isIcon ? 100 : 95]).paddingInner(0.12);
    svg.selectAll('.bars').remove();
    const bars = svg.append('g').attr('class', 'bars');
    const rect = bars.selectAll('rect').data(dataOrArray).enter().append('rect').attr('height', `${yScale.bandwidth()}%`).attr('width', d => `${xScale(d.data)}%`).attr('y', d => `${yScale(d.label)}%`).attr('x', 0).attr('fill', d => color(d.data));
    if (!this.args.isIcon) {
      ...
      }).style('font-size', '.8rem').attr('text-anchor', 'end').attr('text-align', 'right').attr('alignment-baseline', 'central').attr('y', d => `${yScale(d.label) + yScale.bandwidth() / 2}%`).attr('x', d => `${xScale(d.data) - 3}%`).text(d => d.label);
...
setComponentTemplate(TEMPLATE, SimpleChartHorzBar);
