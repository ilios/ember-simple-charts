...
class SimpleChartHorzBar extends Component {
  ...
  paint = modifier((element, [data, isIcon, isClickable, hover, leave, onClick]) => {
    ...
    if (!isIcon) {
      const text = bars.selectAll('text').data(data).enter().append('text').style('fill', d => sliceColor(d.data, color)).style('font-size', '.8rem').attr('text-anchor', 'end').attr('text-align', 'right').attr('dominant-baseline', 'central').attr('y', d => `${yScale(d.label) + yScale.bandwidth() / 2}%`).attr('x', d => `${xScale(d.data) - 3}%`).text(d => d.label);
