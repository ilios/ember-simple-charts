...
class SimpleChartBar extends Component {
  ...
  paint = modifier((element, [data, isIcon, isClickable, hover, leave, onClick]) => {
    ...
    if (!isIcon) {
      const text = bars.selectAll('text').data(data).enter().append('text').style('fill', d => sliceColor(d.data, color)).style('font-size', '.8rem').attr('text-anchor', 'middle').attr('x', d => `${xScale(d.label) + xScale.bandwidth() / 2}%`).attr('y', d => `${110 - yScale(d.data)}%`).text(d => d.data);
