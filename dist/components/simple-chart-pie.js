...
class SimpleChartPie extends Component {
  ...
  paint = modifier((element, [data, isIcon, isClickable, hover, leave, onClick, containerHeight, containerWidth]) => {
    ...
    if (!isIcon) {
      ...
      const text = chart.selectAll('.slice').append('text').style('fill', d => sliceColor(d.data.data, color)).style('font-size', '.8rem').attr('transform', d => 'translate(' + createLabelArc.centroid(d) + ')').attr('text-anchor', 'middle').text(d => d.data.label);
