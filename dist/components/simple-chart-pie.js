...
class SimpleChartPie extends Component {
  ...
  draw(element, [elementHeight, elementWidth, data]) {
    ...
    chart.selectAll('path.slicepath').transition().ease(easeLinear).duration(500).attrTween('d', b => {
      b.innerRadius = 0;
      const i = interpolate({
        startAngle: 0,
        endAngle: 0
    ...
    if (!this.args.isIcon) {
      ...
      }).style('font-size', '.8rem').attr('transform', d => 'translate(' + createLabelArc.centroid(d) + ')').attr('text-anchor', 'middle').text(d => d.data.label);
