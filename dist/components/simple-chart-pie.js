...
var TEMPLATE = precompileTemplate("<svg\n  xmlns=\'http://www.w3.org/2000/svg\'\n  class=\'simple-chart-pie{{if this.isLoading \" loading\" \" loaded\"}}\'\n  height={{@containerHeight}}\n  width={{@containerWidth}}\n  {{this.paint this.dataOrArray @isIcon @isClickable @hover @leave @onClick @containerHeight @containerWidth}}\n  ...attributes\n></svg>");

class SimpleChartPie extends Component {
  static {
    g(this.prototype, "loadingPromise", [tracked]);
  }
  #loadingPromise = (i(this, "loadingPromise"), undefined);
  get loadingData() {
  ...
  paint = modifier((element, [data, isIcon, isClickable, hover, leave, onClick, containerHeight, containerWidth]) => {
    ...
    if (!isIcon) {
      ...
      const text = chart.selectAll('.slice').append('text').style('color', d => sliceColor(d.data.data, color)).style('font-size', '.8rem').attr('transform', d => 'translate(' + createLabelArc.centroid(d) + ')').attr('text-anchor', 'middle').text(d => d.data.label);
...
setComponentTemplate(TEMPLATE, SimpleChartPie);

export { SimpleChartPie as default };
//# sourceMappingURL=simple-chart-pie.js.map
